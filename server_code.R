...
observe({
  if(input$aggregate) {
    nam <- if(!input$applyselect || is.null(input$vars)) # Note that observe() environemnts are independent, so we can use the same variable 'nam' here without affecting the previous observe()
        fnames(datasetInput()) else if(input$excselect) 
        fsetdiff(fnames(datasetInput()), input$vars) else input$vars
    ...
    if(input$applyaggregate && input$aggregate && !is.null(input$aggIDvars)) {
        out <- collapv(out, input$aggIDvars, input$aggfuns, fmode, # If applicable, categorical data is aggregated with the statistical mode (which defaults to the first value if all elements are distinct or identical)
                       ...
                       return = if(length(input$aggfuns) == 1L || !input$aggregate_long) # When multiple functions are selected, a long format can be returned. categorical data is duplicated if applicable.
                         "wide" else if(!input$aggregatedropcat && length(fsetdiff(cat_vars(out, "names"), input$aggIDvars))) "long_dupl" else "long") 
        if(length(input$aggfuns) > 1L) {
          if(input$aggregate_long) {
            out$Function <- substr(out$Function, 2L, 10000L) 
          } else {
            id <- match(input$aggIDvars, fnames(out))
            names(out)[-id] <- substr(fnames(out)[-id], 2L, 10000L)
          }
        }
    }
...
output$summary <- renderTable(mysummary(DATA()), sanitize.text.function = function(x) x) #, style = "overflow-y: scroll;overflow-x: scroll;")
...
output$corrmat<-renderPlot({ #GGCORRPLOT
  ...
  diag(r$P) <- 0
  ggcorrplot(r$r, p.mat = r$P, lab = TRUE,
                         hc.order = !input$corrclust, type='full', outline.col = "white", 
                         lab_size = input$corfontsize, pch.cex = input$corfontsize+1,
                         insig = if(isTRUE(input$insigblank)) "blank" else "pch") + coord_fixed(0.65) # show.diag=TRUE
...
output$iplorcorr<-iplot_render({ # INTERACTIVE CORRPLOT
  ...
  iplotCorr(cleandat, group = NULL, reorder = !input$corrclust2, 
